...
from src.utils import get_sim_path, get_sim_path_sabr
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, cash_flow_flag=0, dg_random_seed=None, num_sim=500002, sabr_flag = False,
        continuous_action_flag=False, spread=0, kappa = 0.01, cost_multiplier = 0, init_ttm=10, trade_freq=0.2, num_contract=1, mu =0, vol = 0.01*np.sqrt(250), S = 100, K = 100, r = 0, q = 0):
        ...
        self.num_state = 3  # price, strike price, position, time to maturity
    ...
    def step_profit_loss(self, action):
        ...
        # calculate period reward (part 1)
        reward = -(price - current_price) * position - cost(position- current_position,self.cost_multiplier )
        # reward = (price - current_price) * position - np.abs(current_position - position) * current_price * self.spread

        # if tomorrow is end of episode
        if self.t == self.num_period - 1:
            done = True
            reward = reward + (max(price - self.strike_price, 0) - current_option_price) * self.num_contract * 100 - cost(position,self.cost_multiplier)
        ...
        reward = reward - 0.5*self.kappa*(reward**2)
